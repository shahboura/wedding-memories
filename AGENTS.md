...
- **Quarantine exposure (new bug):** rejected uploads (failed magic-byte validation) are written to `{LOCAL_STORAGE_PATH}/quarantine/`, which `walkDirectory()` never skipped. They appeared in the gallery as a "quarantine" guest and were publicly downloadable via `/api/media/quarantine/...`; `?guest=quarantine` also bypassed the root filter. Now excluded from listing (dir-level skip + `startsWith('quarantine/')`) and refused with 404 in the media route. Not a token bypass — every API route still enforces the event token; it was an authorization-scope bug. Quarantine was then relocated outside the served root via `QUARANTINE_PATH` (default: sibling of the media root) with a dedicated Docker volume; the listing/serving exclusions remain as defense-in-depth.
